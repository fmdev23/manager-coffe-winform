...
namespace QLQuanCafe
{
    public partial class fTableManager : Form
    {
        ...
        string currentUserName; // lưu user hiện tại
        int currentTableID = -1; // lưu bàn hiện tại
        int selectedFoodID = -1; // lưu món đang chọn trong bill
        ...
        bool CheckTableHasBill(int tableID) // kiểm tra bàn có bill chưa
        {
            string query = @"SELECT COUNT(*) FROM Bill WHERE IDTable = @id AND billStatus = 1";
        ...
        void LoadBillByTable(int tableID) // load bill theo bàn
        {
            lsvBill.Items.Clear();
        ...
        void LoadCategory() // load danh mục món
        {
        ...
        void AddOrUpdateBillInfo(int billID, int foodID, int quantity) // thêm hoặc cập nhật món trong bill
        {
        ...
        void LoadEmptyTableToSwitch() // load bàn trống vào combobox chuyển bàn
        {
            string query = @"SELECT ft.tableID, ft.tableName 
                            FROM FoodTable ft 
                            WHERE NOT EXISTS (SELECT 1 FROM Bill b WHERE b.IDTable = ft.tableID AND b.billStatus = 1)";
    ...
    FROM FoodTable ft";
        ...
        private void btnDelFood_Click(object sender, EventArgs e) // xóa món khỏi bill
        {
            if (currentTableID == -1 || selectedFoodID == -1) return;

            DialogResult rs = MessageBox.Show("Bạn có chắc muốn xóa món này?", "Xác nhận", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
        ...
        private void lsvBill_SelectedIndexChanged(object sender, EventArgs e) // load thông tin món khi chọn món trong bill
        {
        ...
        void FoodField_Changed(object sender, EventArgs e) // theo dõi thay đổi số lượng món
        {
            ...
            bool changed = (int)nmFoodCount.Value != originalQuantity;
        ...
        {
            string query = "SELECT Type FROM Account WHERE userName = @user";
            ...
            using (SqlConnection conn = new SqlConnection(connection_string_sql))
            {
                ...
                return Convert.ToBoolean(result); // true = admin, false = staff
            }
        }
        ...
        private void btnCheckOut_Click(object sender, EventArgs e) // thanh toán
        {
            if (currentTableID == -1)
            {
            ...
            using (SqlConnection conn = new SqlConnection(connection_string_sql)) // lấy bill hiện tại
            {
            ...
            // ĐÓNG BILL
            string qCheckout = @"UPDATE Bill SET billStatus = 0, dateCheckOut = GETDATE() WHERE billID = @billID";
