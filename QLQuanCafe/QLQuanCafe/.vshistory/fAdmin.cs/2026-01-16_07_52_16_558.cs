...
namespace QLQuanCafe
{
    public partial class fAdmin : Form
    {
        ...
        void LoadTableStatus() // load trạng thái bàn vào combobox
        {
            ...
            cbTableStatus1.DataSource = dt;
            cbTableStatus1.DisplayMember = "Text";
            cbTableStatus1.ValueMember = "Value";
        }
        ...
        private void dtgvTable_CellClick(object sender, DataGridViewCellEventArgs e) // khi click vào 1 dòng trong datagridview table
        {
            ...
            cbTableStatus1.SelectedValue = Convert.ToBoolean(row.Cells["tableStatus"].Value);
        ...
        //XU LY FOOD -----------------------------------------------------------------------------
        void LoadFood() // load dữ liệu food từ DB lên DataGridView
        {
            using (SqlConnection conn = new SqlConnection(connection_string_sql))
            {
                string sql = @"SELECT f.foodID, f.foodName, f.foodPrice, f.IDCategory, fc.foodCateName, f.foodImage
                               FROM Food f
