...
namespace QLQuanCafe
{
    public partial class fAdmin : Form
    {
        ...
        private void btnShowTKDoanhThu_Click(object sender, EventArgs e)
        {
        ...
        void LoadFood()
        {
            using (SqlConnection conn = new SqlConnection(connection_string_sql))
            {
                string sql = @"SELECT foodID, foodName, foodPrice, IDCategory, foodImage
                       FROM Food";
            ...
            dtgvFoodDrink.Columns["IDCategory"].HeaderText = "Danh mục";
        }
        ...
        private void btnFoodAdd_Click(object sender, EventArgs e)
        {
            ...
            if (cbFoodCategory.SelectedValue == null || cbFoodCategory.SelectedIndex == 0)
            {
            ...
            using (SqlConnection conn = new SqlConnection(connection_string_sql))
            {
                ...
                checkCmd.Parameters.AddWithValue("@name", txbFoodName.Text.Trim());
                ...
                if (exist > 0)
                {
                    MessageBox.Show("Món này đã tồn tại, vui lòng đặt tên khác");
                    txbFoodName.Focus();
                    return;
                }
            }
            ...
            string query = @"INSERT INTO Food
                     (foodName, foodPrice, IDCategory, foodImage)
                     VALUES
                     (@name, @price, @cat, @image)";
            ...
            using (SqlConnection conn = new SqlConnection(connection_string_sql))
            {
                ...
                try
                {
                    ...
                    LoadFood();      // reload grid
                    ClearFoodForm(); // clear input
                }
                catch (Exception ex)
                {
